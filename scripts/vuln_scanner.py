...
RULES = {
    "python": [
        ...
        ("SEC027", "HIGH",   r'(\.\./|\.\.\\|\bpath\.(join|resolve)\b|\bopen\s*\()',
                             "Possible path traversal risk"),
    ...
    "javascript": [
        ...
        ("SEC026", "HIGH",   r'\b(Handlebars|Mustache)\.compile\s*\(|\b_\.template\s*\(|\bng-bind-html\b|\$sce\.trustAsHtml\s*\(|\bv-html\s*=|\{\{\{[^}]+\}\}\}',
                             "Client-side template injection risk"),
        ("SEC027", "HIGH",   r'(\.\./|\.\.\\|\bpath\.(join|resolve)\b|\bfs\.(readFileSync|readFile|open)\b)',
                             "Possible path traversal or unsafe path-based data loading"),        ("SEC004", "HIGH",   r'(query|sql)\s*[=+]\s*[`"\'].*\$\{', "Possible SQL injection via template literal"),
    ...
    "php": [
        ...
        ("SEC034", "MEDIUM", r'\b(pathinfo\s*\(.*PATHINFO_EXTENSION|mime_content_type\(|finfo_file\()\b',
                             "File type/extension detection is in place; ensure allowlist is used and not bypassed"),
        ("SEC027", "HIGH",   r'(\.\./|\.\.\\|\b(\$?)(__DIR__|dirname\(|realpath\(|pathinfo\(|basename\()\b).*\+|\$\w+\s*\.)',
                             "Possible path traversal risk"),
    ...
    "java": [
        ...
        ("SEC027", "HIGH",   r'(\.\./|\.\.\\|\bPaths?\.(get|resolve)|new File\(|FileInputStream\(|Files\.readAllBytes\()',
                             "Possible path traversal risk"),
    ...
    "go": [
        ...
        ("SEC026", "HIGH",   r'\b(template\.Execute|template\.Parse|html/template|text/template)\b',
                             "Possible template injection risk"),
        ("SEC027", "HIGH",   r'(\.\./|\.\.\\|\bpath\.(Join|Clean|Dir|Base)\b|\bfilepath\.(Join|Clean)\b|io\.ReadFile\()',
                             "Possible path traversal risk"),
    ...
    "bash": [
        ...
        ("SEC027", "HIGH",   r'(\.\./|\.\.\\|\b((cat|ls|find)\s+)?\$\w+\b|\$\{.*\})',
                             "Possible path traversal risk"),
        ("SEC009", "MEDIUM", r'http://',                     "Insecure HTTP protocol"),
    ],
}
